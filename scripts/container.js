...
ngps.findContainer = function(uid){
	return ngps.containerData.reffTree[uid];
}

ngps.setZoomLevelForContainers = function(zl){
	ngps.containerData.zoomLevel = 1/zl;
}

ngps.container = function(_properties,_parent)
{
	function addToTree(container) {
		//console.log("#Adding container to tree:"+container.UID);
		ngps.containerData.reffTree[container.UID] = container;
	}
	function removeFromTree(container) {
		//console.log("#Deleting container from tree:"+container.UID);
		delete ngps.containerData.reffTree[container.UID];
	}
	...
	this.show = function(bringToFront)
	{
		...
		if(bringToFront)
			this.DOMreference.style.zIndex = ngps.containerData.containerIndex+1;
		//EVENT
	...
	this.move = function(dx,dy,noevent)//SLOW  ~ 1 ms to exec
	{
		...
		if(ngps.containerData.cameraCtx){
			var zl = 1/ngps.containerData.cameraCtx.czoomLevel;
			dx *= zl;
			dy *= zl;
		}
	...
	//{
		...
		if( this.properties['UID'] ) {
			...
			if(ngps.containerData.containerIndex < this.UID)
				ngps.containerData.containerIndex = this.UID + 1;
		}
		else
			this.UID = ngps.containerData.containerIndex++;
		//store child object
		if(this.parent)
		...
		if(this.properties['top'])
			this.DOMreference.style.top = this.properties['top'];
...
var containerData = ngps.containerData;
var findContainer = ngps.findContainer;
var setZoomLevelForContainers = ngps.setZoomLevelForContainers;
this.container = ngps.container;
/////////////////////////////////////////