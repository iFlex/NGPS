...
*		Must implement callbacks: onMoved, onScaled, onRotated
*
*	Available events:
*		mouseDown
*		mouseMove
...
Interactive.onMouseDown = function( e , ctx )
{
	...
	if( ctx.propagation == 0 )
	{
		...
		if(!Interaction.origin)
		{
			...
			cli.showPrompt("Registered origin:"+utils.debug(ctx," "));
		}
		if(ctx.onMouseDown)
...
Interactive.onMouseMove = function(e, ctx)
{
	...
	if( Interaction.origin )
	{
		if( ctx.UID != Interaction.origin.UID && ctx.UID == factory.root.UID )
		{
		...
		if( ctx.propagation == 0)
		{
			cli.showPrompt("origin move:"+utils.debug(ctx," "));
...
Interactive.onMouseUp = function( e , ctx )
{
	...
	if(Interaction.origin)
	{
		...
		if(ctx.propagation == 0)
		{
				...
				cli.showPrompt("Origin end:"+utils.debug(ctx," "));
...
Interactive.touchstart = function( e , ctx)
{
	...
	cli.showPrompt("Touch Start:"+utils.debug(ctx," "));
	if( ctx.propagation == 1 )
		return true;
...
Interactive.touchmoved = function( e , ctx)
{
	...
	cli.showPrompt("Touch move:"+utils.debug(ctx," "));
...
Interactive.touchend = function( e, ctx){
	...
	cli.showPrompt("Touch end:"+utils.debug(ctx," "));
	if( ctx.propagation == 1 )
		return true;
...
{
	//one finger events
    obj.DOMreference.ontouchstart = obj.touchstart;
	obj.DOMreference.ontouchmove = obj.touchmoved;
	obj.DOMreference.ontouchend = obj.touchend;
	obj.DOMreference.ontouchcancel = obj.touchend;
	obj.DOMreference.ontouchleave = obj.touchend;
	$("#"+obj.ID);//.swipe( { fingers:'all', swipeStatus:function(){}, allowPageScroll:"none"} );
}

Interactive.disableMobile = function (obj)
{
	//one finger events
    obj.DOMreference.ontouchstart = null;
	obj.DOMreference.ontouchmove = null;
	obj.DOMreference.ontouchend = null;
	obj.DOMreference.ontouchcancel = null;
	obj.DOMreference.ontouchleave = null;
	$("#"+obj.ID);//.swipe( { fingers:'all', swipeStatus:function(){}, allowPageScroll:"auto"} );
}
    ...
    if( d )
	{
  	...
  	else
  	{
  		if( this.isInteractive )
  		{
		  	...
		  	this.disableMobile( this );
		}
  	}
}
Interactive.suspendInteraction = function()
{
	if(this.isInteractive)
	{
		this._onMoved = this.onMoved;
		this.onMoved = function(){};
	}
}
Interactive.resumeInteraction = function()
{
	if(this.isInteractive)
	{
		this.onMoved = this._onMoved;
		delete this._onMoved;
	}
}